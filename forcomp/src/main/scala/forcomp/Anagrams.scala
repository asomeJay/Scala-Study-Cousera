...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toList.groupBy(c => c).map(x => (x._1, x._2.length)).toList.sorted
  ...
  def wordAnagrams(word: Word): List[Word] = dictionaryByOccurrences.withDefaultValue(List())(wordOccurrences(word)).sorted
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = collection.mutable.Map(x map {i => i._1 -> (i._2 - y.find(yy => i._1 == yy._1).getOrElse((0,0))._2)}: _*)
    .filter(p => p._2 > 0)
